...
from ea_engine import create_scenario, random_placement, compute_fitness, evolve_one_generation
...
class PCBOptimizerApp:
    ...
    def _build_gui(self):
        # Top-Level Layout (Modern Flexbox-style packing)
        main_frame = tk.Frame(self.root, bg="#121214")
        main_frame.pack(fill=tk.BOTH, expand=True, padx=15, pady=15)

        # LINKER BEREICH: Das PCB-Sichtfenster
        ...
        # RECHTER BEREICH: Dashboard & Controls
        ...
        self.lbl_fit = self._add_stat_row(stats_card, "Beste Länge:", "Initializing...")
    ...
    def reset(self):
        ...
        self.fitness_vals = [compute_fitness(g, self.netlist) for g in self.population]
    ...
    def _auto_run_loop(self):
        if not self.is_running:
            return
        # Rechnet 5 Generationen pro grafischem Frame für bessere Performance
        for _ in range(5):
            self.population, self.fitness_vals = evolve_one_generation(self.population, self.fitness_vals, self.netlist)
            self.generation += 1
    ...
    def _update_best(self):
        best_idx = min(range(len(self.fitness_vals)), key=lambda i: self.fitness_vals[i])
    ...
    def _update_status(self):
        ...
        if self.best_fitness == float('inf'):
            self.lbl_fit.config(text="⚠️ Infeasible", fg="#D94343")
        else:
            self.lbl_fit.config(text=f"{self.best_fitness * NM_TO_MM:.1f} mm", fg="#4E9F3D")
        
        inf_count = sum(1 for f in self.fitness_vals if f == float('inf'))
        self.lbl_inf.config(text=f"{inf_count} / {POP_SIZE}", fg="#E4E4E7" if inf_count == 0 else "#FFB344")
        ...
        for net_id, connections in self.netlist.items():
            ...
            for i in range(len(net_pins)):
                for j in range(i + 1, len(net_pins)):
                    px1, py1 = self._nm_to_canvas(*net_pin_coords[i] if 'net_pin_coords' in locals() else net_pins[i])
                    px2, py2 = self._nm_to_canvas(*net_pin_coords[j] if 'net_pin_coords' in locals() else net_pins[j])
